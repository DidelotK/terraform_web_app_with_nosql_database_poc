...
resource "azurerm_network_security_group" "security_group_web" {
  ...
  security_rule {
    ...
    direction                  = "Inbound"
    access                     = "Allow"
    source_address_prefix      = "*" // #37.171.151.170
    source_port_range          = "*"
...
resource "null_resource" "ansible_provisioner" {
  provisioner "local-exec" {
    command = "sleep 30; ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.web_app["admin_user"]} --private-key ../ssh-keys/deployer-key -i '${data.azurerm_public_ip.data_debug_public_ip.ip_address},' ../ansible/web_server.deploy.yml"
  }

  depends_on = [
    "data.azurerm_public_ip.data_debug_public_ip",
    "azurerm_cosmosdb_account.web_app_database"
  ]
}
...
output "azure_cosmosdb_primary_key" {
  value = "${azurerm_cosmosdb_account.web_app_database.primary_master_key}"
}

output "azure_cosmosdb_uri" {
  value = "${azurerm_cosmosdb_account.web_app_database.id}"
}
